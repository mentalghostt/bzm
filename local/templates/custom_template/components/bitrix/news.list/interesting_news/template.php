							...
							<div class="title-container mb-1 w-100" style="max-width: 100%; overflow: hidden;">
								<? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])): ?>
									<h6 class="card-title m-0"
										style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; min-height: 3em; line-height: 1.5em;">
										<a href="<?= $arItem["DETAIL_PAGE_URL"] ?>"
											class="text-decoration-none"><?= $arItem["NAME"] ?></a>
									</h6>
								<? else: ?>
									<h6 class="card-title text-truncate"><?= $arItem["NAME"] ?></h6>
								<? endif; ?>
							</div>
						<? endif; ?>

						<? if ($arParams["DISPLAY_DATE"] != "N" && $arItem["DISPLAY_ACTIVE_FROM"]): ?>
							<div class="text-muted small"><?= $arItem["DISPLAY_ACTIVE_FROM"] ?></div>
						<? endif ?>
